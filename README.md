...
- 运营 dashboard 可直接把高风险 workspace 移交到 CRM / 人工跟进并暂停自动重新入队，也可导出 recovery queue CSV 与 CRM handoff CSV，或直接同步 CRM API case、强制推送外部 recovery webhook / Ops Slack；系统会持续记录 CRM case 外部 id、CRM/API/webhook 尝试次数、最近一次尝试、下次自动重试窗口、dead-letter 状态，以及内部 Slack 告警是否已经成功送达
