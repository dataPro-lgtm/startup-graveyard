...
> 一个已经具备结构化案例库、管理后台、基础 ingestion，以及带 session / prompt telemetry 的 Copilot 能力的 alpha 版 failure research product。
...
- 案例列表、关键词搜索、多维筛选
- 案例详情页：公司概览、时间线、失败因子、证据来源、核心教训
- 相似案例推荐
- Failure Copilot：基于案例知识库的 grounded Q&A，支持多会话线程、pinned context、回答反馈与 run telemetry
...
- taxonomy 规范化与历史回填
- Copilot prompt versioning、run-level token/cost telemetry
...
- `M3` 商业化闭环：Free / Pro / Team、收藏、watchlist、导出、账单生命周期
- `M4` 平台化：OTel、worker、Redis、对象存储、nightly regression、数据质量报表
...
- Copilot 已有 session、上下文 pin、反馈回路、prompt version 和 token/cost 追踪，但评测集、离线回归和运营 dashboard 还没补齐
