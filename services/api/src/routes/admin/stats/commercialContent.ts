...
import {
  followUpStateFromWorkspace,
  followUpStatePriority,
  followUpStateTitle,
  nextFollowUpAt,
  recoveryStageFromCommercialTouch,
  recoveryStageTitle,
} from './shared.js';
...
export async function fetchCommercialStats(app: FastifyInstance): Promise<CommercialAdminMetrics> {
  const [
    subscriptionStats,
    billingFunnelStats,
    watchlistStats,
...
export async function fetchContentStats(pool: Pool): Promise<ContentStatsResult> {
  const [
    summaryRes,
    industryRes,
    countryRes,
