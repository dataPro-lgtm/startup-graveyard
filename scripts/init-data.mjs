...
// 检查文件是否存在且为空
const fileExists = fs.existsSync(STARTUPS_FILE);
const isEmpty = fileExists && fs.readFileSync(STARTUPS_FILE, 'utf-8').trim() === '[]';

if (!fileExists || isEmpty) {
  console.log('Initializing default data...');
  fs.writeFileSync(STARTUPS_FILE, JSON.stringify(defaultStartups, null, 2), 'utf-8');
  console.log(`✓ Imported ${defaultStartups.length} default startups`);
} else {
  console.log('Data file already contains data, skipping initialization');
